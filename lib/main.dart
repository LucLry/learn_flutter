          ...
          Expanded(
            child: LayoutBuilder(
              builder: (context, constraints) {
                // Use 2 columns if width >= 600, else 1 column
                int columns = constraints.maxWidth >= 600 ? 2 : 1;
                return GridView.count(
                  crossAxisCount: columns,
                  childAspectRatio: 5, // Make tiles wide and short
...
class BigCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      // refactor => Wrap with widget
      color: theme.colorScheme.primary,
      child: Padding(
        // refactor => Wrap with padding
        padding: const EdgeInsets.all(20),
        child: Text(
          pair.asLowerCase,
          style: style,
          semanticsLabel:
              "${pair.first} ${pair.second}", //this is for screen reader to read the right words
        ),
      ),
    );
  }
